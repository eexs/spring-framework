...
public class ContextLoader {
		...
		try {
			...
			if (this.context instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					// <4.2> 无父容器，则进行加载和设置。默认情况下，#loadParentContext(ServletContext servletContext) 方法，返回 null
			...
			// <5> 记录 context 在 ServletContext 中。这样，如果 web.xml 如果定义了多个 ContextLoader ，就会在 <1> 处报错。
			servletContext.setAttribute(WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE, this.context);

			// <6> 记录到 currentContext 或 currentContextPerThread 中，差异在于类加载器的不同。
			//目前currentContext、currentContextPerThread这两个变量，在销毁 Spring WebApplicationContext 容器时会用到。
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		...
		// 情况一，如果值非空，则获得该类 (从 ServletContext 获得配置的 context 类)
		...
		// 情况二，从 defaultStrategies 获得该类 (从 ContextLoader.properties 获得配置的 context 类)
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		// <1> 如果 context 使用了默认编号，则重新设置 id 属性。默认情况下，我们不会对 context 设置编号，所以会执行进去。
...
//		</context-param>
		...
		// <5> 执行自定义初始化 context 非关键方法，暂时忽略。
		customizeContext(sc, wac);
		// 刷新 context ，执行初始化。此处，就会进行一些的 Spring 容器的初始化。
		wac.refresh();
	}
