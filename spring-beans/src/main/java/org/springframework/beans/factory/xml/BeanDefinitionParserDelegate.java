...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		if (beanDefinition != null) {
			//如果id、aliases两者都为空，则根据默认规则来设置 beanName 。
			...
			if (!StringUtils.hasText(beanName)) {
				try {
					if (containingBean != null) {
						...
						// <3.3> 生成唯一的 beanName
						// 如果我们不定义 id 和 name，默认生成如下：
						//   1. beanName 为：com.xxx.MessageServiceImpl#0
						//   2. beanClassName 为：com.xxx.MessageServiceImpl
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		if ((hasRefAttribute && hasValueAttribute) || // 1. ref 和 value 都存在
				((hasRefAttribute || hasValueAttribute) && subElement != null)) {// 2. ref 和 value 存在一，并且 subElement 存在
			error(elementName +
	...
	public BeanDefinition parseCustomElement(Element ele, @Nullable BeanDefinition containingBd) {
		// <1> 获取 namespaceUri （获取xml配置文件中的命名空间http://www.springframework.org/schema/aop）
		String namespaceUri = getNamespaceURI(ele);
		if (namespaceUri == null) {
			return null;
		}
		// <2> 根据 namespaceUri 获取相应的 Handler （根据命名空间找到命名空间处理类AopNamespaceHandler）
		NamespaceHandler handler = this.readerContext.getNamespaceHandlerResolver().resolve(namespaceUri);
		if (handler == null) {
			error("Unable to locate Spring NamespaceHandler for XML schema namespace [" + namespaceUri + "]", ele);
			return null;
		}
		// 调用自定义的 Handler 处理（解析命名空间支持的标签）、（AOP标签解析请看：AopNamespaceHandler）
