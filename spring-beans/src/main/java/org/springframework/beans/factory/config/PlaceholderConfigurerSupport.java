...
public abstract class PlaceholderConfigurerSupport extends PropertyResourceConfigurer
	...
	protected void doProcessProperties(ConfigurableListableBeanFactory beanFactoryToProcess,
		...
		//BeanDefinitionVisitor 是 BeanDefinition 的访问者，我们通过它可以实现对 BeanDefinition 内容的进行访问，并且通过StringValueResolver 解析字符串
		// 内容很多，例如 Scope、PropertyValues、FactoryMethodName 等等。
		BeanDefinitionVisitor visitor = new BeanDefinitionVisitor(valueResolver);

		// <2.2> 得到该容器的所有 BeanName，然后对其进行访问（ #visitBeanDefinition(BeanDefinition beanDefinition) 方法）。
