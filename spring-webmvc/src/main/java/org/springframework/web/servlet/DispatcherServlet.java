...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				// <3> 获得请求对应的 HandlerExecutionChain 对象。包含处理器( handler )和拦截器们( HandlerInterceptor 数组 )
				mappedHandler = getHandler(processedRequest);
				// <3.1> 如果获取不到，则调用 #noHandlerFound方法 根据配置抛出异常或返回 404 错误。
				...
				// <5> 前置处理 拦截器 (调用 HandlerInterceptor#preHandle)
				...
				// <9> 后置处理 拦截器 (调用 HandlerInterceptor#postHandle)
			...
			// <12> 已完成 拦截器 (调用 HandlerInterceptor#afterCompletion)
	...
	private void processDispatchResult(HttpServletRequest request, HttpServletResponse response,
		...
		if (mv != null && !mv.wasCleared()) {
			...
			// <3.2> 清理请求中的错误消息属性
			// 原因在： #processHandlerException方法中，会调用 WebUtils#exposeErrorRequestAttributes方法，设置请求中的错误消息属性。
			if (errorView) {
		...
		// <5> 已完成处理 拦截器 (调用 HandlerInterceptor#afterCompletion)
		if (mappedHandler != null) {
