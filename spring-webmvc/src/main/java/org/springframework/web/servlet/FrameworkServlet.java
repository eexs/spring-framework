...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected WebApplicationContext initWebApplicationContext() {
		...
		if (this.webApplicationContext != null) {
			...
			if (wac instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					// 配置和初始化 wac
					// 情况一 + wac 未激活。 (通过SourceFilteringListener触发DispatcherServlet#onRefresh方法)
		...
		if (wac == null) {
			...
			// 情况三。(通过SourceFilteringListener触发DispatcherServlet#onRefresh方法)
			wac = createWebApplicationContext(rootContext);
		}

		// <3> 如果未触发刷新事件，则主动触发刷新事件。(是否有接收到 ContextRefreshedEvent 事件。)
		// 情况一 + wac 已激活。 情况二 这两种情形，此时 refreshEventReceived 为 false 会触发DispatcherServlet#onRefresh方法
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		// <3> 添加监听器 SourceFilteringListener 到 wac 中 (用于触发DispatcherServlet#onRefresh方法)
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		// <2.1> 处理 PATCH 请求 (因为 HttpServlet 默认没提供 #doPatch方法)
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			// <9> Locale + RequestAttributes
			resetContextHolders(request, previousLocaleContext, previousAttributes);
			// <10> Locale + RequestAttributes
