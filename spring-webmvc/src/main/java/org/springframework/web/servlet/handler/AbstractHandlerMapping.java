 ...
 */
//HandlerMapping 抽象基类，实现了【获得请求对应的处理器和拦截器们】的骨架逻辑，而暴露 #getHandlerInternal 抽象方法，交由子类实现。
//WebApplicationObjectSupport 抽象类，提供 applicationContext 属性的声明和注入。
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
		implements HandlerMapping, Ordered, BeanNameAware {

	//默认处理器 (默认处理器。在获得不到处理器时，可使用该属性。)
	@Nullable
	 ...
	 */
	//对 WebApplicationObjectSupport 的覆写，继承关系是 WebApplicationObjectSupport => ApplicationObjectSupport => ApplicationContextAware 。
	@Override
